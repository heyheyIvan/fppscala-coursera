...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    def iter(s: TweetSet, resultList: TweetList): TweetList = {
      if (s.isEmpty) resultList
      else {
        val mr = s.mostRetweeted
        iter(s.remove(mr), resultList.append(mr))
      }
    }
    iter(this, Nil)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    ...
    val max = if (leftMax.retweets > rightMax.retweets) leftMax else rightMax
...
trait TweetList {
  ...
  def append(elem: Tweet): TweetList = {
    if (isEmpty) new Cons(elem, Nil)
    else new Cons(head, tail.append(elem))
  }
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(t => google.exists(word => t.text.contains(word)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(t => apple.exists(word => t.text.contains(word)))
